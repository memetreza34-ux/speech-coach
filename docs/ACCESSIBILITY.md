...
`src/RootApp.jsx` verwaltet die sechs Vollbildbereiche zentral:
...
Die sechs Launcher besitzen stabile `data-focus-key`-Kennungen. Dadurch wird nach dem Schließen nicht versucht, auf eine bereits ungemountete DOM-Referenz zurückzukehren. Stattdessen wird der entsprechende neu gerenderte Launcher gesucht und fokussiert.
...
Ein visuell versteckter `aria-live="polite"`-Bereich meldet den Wechsel zwischen Hauptansicht und den sechs Vollbildbereichen.

Die bestehende Oberfläche verwendet bei reinen Icon-Aktionen wie Zurück, Schließen und Sprachausgabe bereits beschreibende `aria-label`-Texte an den zentralen Coach-Headern.
...
Die Launcher-Hover-/Tap-Animationen werden zusätzlich explizit deaktiviert, wenn Framer Motion Reduced Motion meldet.

### Touch-Ziele

Auf Geräten mit grobem Pointer werden zentrale Icon-Buttons auf mindestens 44 × 44 Pixel angehoben.

## Automatische Repository-Gates

`scripts/smoke-check.mjs` verlangt dauerhaft:
...
- mindestens 44 Pixel große zentrale Icon-Touch-Ziele auf groben Pointern
...
1. Hauptseite öffnen.
2. Mit `Tab` alle sechs Launcher erreichen.
