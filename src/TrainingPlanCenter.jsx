...
function PlanHeader({ onClose, source, signedIn }) {
  return (
    <header className="plan-header">
      <div className="plan-header-title"><span><CalendarRange size={21} /></span><div><strong>Dein Trainingsplan</strong><small>Vier Wochen adaptives Kommunikationstraining</small></div></div>
      <div className="plan-header-actions"><div className="plan-storage-badge">{signedIn ? <Cloud size={16} /> : <CloudOff size={16} />}<span>{signedIn ? source === 'cloud' ? 'Cloud-Plan' : 'Wird synchronisiert' : 'Lokal gespeichert'}</span></div><button onClick={onClose} aria-label="Trainingsplan schließen"><X size={20} /></button></div>
    </header>
  )
}

function PlanLoading({ onClose }) {
  return <motion.div className="plan-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><main className="plan-shell"><PlanHeader onClose={onClose} source="local" signedIn={false} /><div className="plan-loading"><LoaderCircle className="plan-spin" size={38} /><h1>Dein Plan wird berechnet …</h1><p>Fähigkeiten, Trainingshistorie und Wochenziel werden ausgewertet.</p></div></main></motion.div>
}
      ...
      <motion.main className="plan-shell" initial={{ opacity: 0, y: 18, scale: 0.99 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.99 }}>
        ...
        <div className="plan-content">
          <section className="plan-hero">
            <div className="plan-progress-ring" style={{ '--plan-progress': `${stats.progressPercent * 3.6}deg` }}><div><strong>{stats.progressPercent}%</strong><span>abgeschlossen</span></div></div>
            <div className="plan-hero-copy"><span className="plan-eyebrow"><Sparkles size={15} /> Adaptiv aus deinem Profil</span><h1>Vier Wochen. Ein klarer Fokus. <span>Messbare Wiederholung.</span></h1><p>Der Plan gewichtet deine schwächsten Fähigkeiten stärker und steigert die Anforderungen von der Grundlage bis zum realistischen Transfer.</p><div className="plan-focus-list">{plan.focusSkills.map((skill, index) => <span key={skill.key}><strong>{index + 1}</strong>{skill.label}{Number.isFinite(skill.baselineScore) && <small>{skill.baselineScore}</small>}</span>)}</div></div>
            <aside className="plan-current-card"><span>Aktuelle Woche</span><strong>{stats.currentWeek} von {plan.durationWeeks}</strong><div><i style={{ width: `${stats.currentWeekTotal ? (stats.currentWeekCompleted / stats.currentWeekTotal) * 100 : 0}%` }} /></div><small>{stats.currentWeekCompleted}/{stats.currentWeekTotal} Einheiten · Start {formatDate(plan.startedOn)}</small></aside>
          </section>
          <section className="plan-summary-grid"><article><CalendarDays size={20} /><span>Planlänge</span><strong>4 Wochen</strong><small>{plan.sessionsPerWeek} Kerneinheiten pro Woche</small></article><article><CheckCircle2 size={20} /><span>Erledigt</span><strong>{stats.completedCount}/{stats.totalCount}</strong><small>automatisch oder manuell markiert</small></article><article><Timer size={20} /><span>Trainingsumfang</span><strong>{stats.completedMinutes}/{stats.totalMinutes} Min.</strong><small>geplante aktive Übungszeit</small></article><article><Trophy size={20} /><span>Ausgangsniveau</span><strong>{plan.baseline.overall || 'Start'}</strong><small>{plan.baseline.sessionCount} Trainings ausgewertet</small></article></section>
